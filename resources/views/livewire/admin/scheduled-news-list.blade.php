<div class="p-6 bg-white rounded shadow">
    <h1 class="text-2xl font-bold mb-6 text-black">Scheduled News</h1>

    @if($scheduledNews->count())
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left border text-black">
                ...
                <tbody>
                    @foreach($scheduledNews as $index => $news)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 border text-xs text-gray-500">
                                {{ $news->scheduled_at ? $news->scheduled_at->format('M d, Y H:i') : 'Not Scheduled' }}
                            </td>
    ...
    @else
        <p class="text-black">No scheduled news found.</p>
    @endif
</div>
